...
# === Setup DB and Admin user ===
def setup_database():
    db.create_all()
    admin_user = User.query.filter_by(username='im_abi').first()
    if not admin_user:
        # اگر کاربری با نام im_abi نیست، ادمین نمیسازیم خودِ روت ثبت‌نام می‌کنه
        pass
...
def register():
    if request.method == 'POST':
        ...
        # اگر اولین کاربر با نام im_abi هست، ادمین کن
        is_admin = False
        if username == 'im_abi' and User.query.filter_by(username='im_abi').count() == 0:
            is_admin = True
...
if __name__ == '__main__':
    # اینجا قبل اجرا، جدول‌ها و دیتا رو ستاپ کن
